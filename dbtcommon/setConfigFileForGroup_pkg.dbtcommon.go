...
with parameters:
command line:2: -group pkg.dbtcommon
...
/*
SetGroupConfigFile_pkg_dbtcommon adds a config file to the set which the param
...
*/
func SetGroupConfigFile_pkg_dbtcommon(ps *param.PSet) error {
...
/*
SetGroupGlobalConfigFile_pkg_dbtcommon adds a config file to the set which the
...
config file, thereby enforcing a global policy.
*/
func SetGroupGlobalConfigFile_pkg_dbtcommon(ps *param.PSet) error {
	dirs := xdg.ConfigDirs()
	if len(dirs) == 0 {
		return nil
	}
